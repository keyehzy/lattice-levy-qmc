...
PathSlice PathCursor::slice(const PathCut &left, const PathCut &right, const PathCutSide left_side,
  ...
  return PathSlice{
      ...
      .events =
          std::span<const JumpEvent>(path_.events).subspan(first_event, last_event - first_event),
  };
}
...
ContinuousPath splice_path_slices(const PathSlice &prefix_slice, const PathSlice &suffix_slice,
  ...
  const std::span<const JumpEvent> prefix =
      std::span<const JumpEvent>(prefix_source.events).first(prefix_slice.first_event);
  events.insert(events.end(), prefix.begin(), prefix.end());
  append_shifted_events(events, replacement.events, prefix_slice.tau0);
  const std::span<const JumpEvent> suffix =
      std::span<const JumpEvent>(suffix_source.events).subspan(suffix_event);
  ...
  ContinuousPath result{
      .duration = prefix_source.duration,
      .start = prefix_source.start,
      .end = std::move(end),
      .events = std::move(events),
  };
  result.validate(result.start.size());
